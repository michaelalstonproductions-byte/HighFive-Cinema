...
JSON_OUT="$OUT_DIR/cloud_library_sync_staging_screenshot_manifest.json"
MD_OUT="$OUT_DIR/cloud_library_sync_staging_screenshot_manifest.md"
DERIVED_DATA="/Volumes/Scratch SSD/XcodeDerivedData/highfive-58-0b-cloud-library-sync-evidence"
...
capture_route "library_sync_local" "$SCREENSHOT_DIR/library_sync_local.png" --hf-skip-onboarding --hf-start-library
capture_route "profile_library_sync" "$SCREENSHOT_DIR/profile_library_sync.png" --hf-skip-onboarding --hf-start-profile
capture_route "saved_titles_sync_boundary" "$SCREENSHOT_DIR/saved_titles_sync_boundary.png" --hf-skip-onboarding --hf-start-movie-detail
capture_route "backend_library_sync_status" "$SCREENSHOT_DIR/backend_library_sync_status.png" --hf-start-backend-status || true

required_failures=()
for entry in "${captures[@]}"; do
  IFS='|' read -r name path state <<< "$entry"
  if [[ "$name" != "backend_library_sync_status" && "$state" != "captured" ]]; then
...
{
  printf -- '# Cloud Library Sync Staging Screenshot Manifest\n\n'
  printf -- '- Upgrade: #058.0B\n'
  printf -- '- Status: %s\n' "$status"
  printf -- '- Build: passed\n'
  printf -- '- Install: passed\n\n'
...
printf -- 'Cloud library sync staging screenshot harness passed.\n'
