...
JSON_OUT="$OUT_DIR/entitlement_cloudflare_descriptor_screenshot_manifest.json"
MD_OUT="$OUT_DIR/entitlement_cloudflare_descriptor_screenshot_manifest.md"
DERIVED_DATA="/Volumes/Scratch SSD/XcodeDerivedData/highfive-61-0b-entitlement-cloudflare-descriptor-evidence"
...
declare -a SHOTS=(
  "movie_detail_cloudflare_descriptor.png|--hf-skip-onboarding --hf-start-movie-detail|required"
  "player_entitlement_descriptor_gate.png|--hf-skip-onboarding --hf-start-player|required"
  "profile_playback_descriptor_readiness.png|--hf-skip-onboarding --hf-start-profile|required"
  "backend_playback_descriptor_status.png|--hf-start-backend-status|optional"
)
...
capture_shot() {
  ...
  xcrun simctl terminate booted "$APP_ID" || true
  if xcrun simctl launch booted "$APP_ID" $args >/tmp/highfive-61-0b-launch.log 2>&1; then
    sleep 3
    if xcrun simctl io booted screenshot "$path" >/tmp/highfive-61-0b-screenshot.log 2>&1 && [[ -s "$path" ]]; then
...
{
  printf -- '# Entitlement-Gated Cloudflare Playback Descriptor Screenshot Manifest\n\n'
  printf -- '- Upgrade: #061.0B\n'
  printf -- '- Status: %s\n' "$status"
  printf -- '- Build: passed\n'
  printf -- '- Install: passed\n\n'
...
printf -- 'Entitlement-gated Cloudflare playback descriptor screenshot harness passed.\n'
