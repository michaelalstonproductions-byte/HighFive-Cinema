...
JSON_OUT="$OUT_DIR/payment_entitlement_staging_screenshot_manifest.json"
MD_OUT="$OUT_DIR/payment_entitlement_staging_screenshot_manifest.md"
DERIVED_DATA="/Volumes/Scratch SSD/XcodeDerivedData/highfive-57-0b-payment-entitlement-evidence"
...
capture_route "movie_detail_entitlement" "$SCREENSHOT_DIR/movie_detail_entitlement.png" --hf-skip-onboarding --hf-start-movie-detail
capture_route "profile_payment_readiness" "$SCREENSHOT_DIR/profile_payment_readiness.png" --hf-skip-onboarding --hf-start-profile
capture_route "vod_entitlement_boundary" "$SCREENSHOT_DIR/vod_entitlement_boundary.png" --hf-skip-onboarding --hf-start-vod-package
capture_route "backend_entitlement_status" "$SCREENSHOT_DIR/backend_entitlement_status.png" --hf-start-backend-status || true

required_failures=()
for entry in "${captures[@]}"; do
  IFS='|' read -r name path state <<< "$entry"
  if [[ "$name" != "backend_entitlement_status" && "$state" != "captured" ]]; then
...
{
  printf -- '# Payment Entitlement Staging Screenshot Manifest\n\n'
  printf -- '- Upgrade: #057.0B\n'
  printf -- '- Status: %s\n' "$status"
  printf -- '- Build: passed\n'
  printf -- '- Install: passed\n\n'
...
printf -- 'Payment entitlement staging screenshot harness passed.\n'
