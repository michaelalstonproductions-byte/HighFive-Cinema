...
SOURCE_JSON="$OUT_DIR/auth_account_staging_source_verification.json"
MANIFEST_JSON="$OUT_DIR/auth_account_staging_screenshot_manifest.json"
SCREENSHOT_VERIFY_JSON="$OUT_DIR/auth_account_staging_screenshot_verification.json"
JSON_OUT="$OUT_DIR/auth_account_staging_evidence_report.json"
MD_OUT="$OUT_DIR/auth_account_staging_evidence_report.md"
...
require_source "HFAuthService" "auth service foundation evidence exists"
require_source "HFLocalAuthAdapter" "local auth adapter evidence exists"
require_source "HFRemoteAuthAdapter" "remote auth adapter evidence exists"
require_source "guard configuration.hasAnyRuntimeConfig else" "remote auth adapter config-gated evidence exists"
require_source "HFSessionState" "session state evidence exists"
require_source "HFAccountIdentity" "account identity evidence exists"
require_source "HFAccountDeletionRequest" "account deletion boundary evidence exists"
require_source "HFAccountExportRequest" "account export boundary evidence exists"
require_source "Sign in with Apple requirement pending" "Sign in with Apple requirement evidence exists"
require_source "hf.account.panel" "Profile account panel evidence exists"
require_source "Local Account Mode" "Local Account Mode fallback evidence exists"
require_source "Missing Auth Credentials" "Missing Auth Credentials evidence exists"
require_source "Auth Configured" "Auth Configured evidence exists"
require_source "No OAuth tokens" "no live OAuth evidence exists"
require_source "These are staging boundary states" "no live account deletion/export evidence exists"
...
SCREENSHOT_PATHS=(
  "$OUT_DIR/screenshots/profile_account_local.png"
  "$OUT_DIR/screenshots/backend_status_auth_readiness.png"
  "$OUT_DIR/screenshots/home_account_backend_boundary.png"
  "$OUT_DIR/screenshots/creator_account_boundary.png"
)
...
KNOWN_LIMITATIONS=(
  "evidence only"
  "auth/account staging foundation only"
  "app stays Local Account Mode unless runtime auth config is provided"
  "no committed secrets"
  "no hardcoded production URLs"
  "no live OAuth"
  "no live Sign in with Apple implementation"
  "no token storage"
  "no live account deletion/export"
  "no live cloud sync"
...
{
  echo "# Auth Account Staging Evidence Report"
  echo
  echo "- Upgrade: #055.0B"
  echo "- Status: $STATUS"
