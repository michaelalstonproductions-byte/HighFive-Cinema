...
OUT_ROOT="/Volumes/Scratch SSD/highfive-phase-18-0g-rooms-qa"
...
REPORT_JSON="$OUT_ROOT/highfive_rooms_screenshot_verification.json"
REPORT_MD="$OUT_ROOT/highfive_rooms_screenshot_verification.md"
...
{
  printf '# HighFive Rooms Screenshot Verification\n\n'
...
if [[ "$FAIL_COUNT" -gt 0 ]]; then
  printf 'HighFive Rooms screenshot verification failed. See:\n%s\n%s\n' "$REPORT_JSON" "$REPORT_MD" >&2
  exit 1
fi

printf 'HighFive Rooms screenshot verification passed. Reports:\n%s\n%s\n' "$REPORT_JSON" "$REPORT_MD"
