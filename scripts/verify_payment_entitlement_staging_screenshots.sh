...
MANIFEST_JSON="$OUT_DIR/payment_entitlement_staging_screenshot_manifest.json"
JSON_OUT="$OUT_DIR/payment_entitlement_staging_screenshot_verification.json"
MD_OUT="$OUT_DIR/payment_entitlement_staging_screenshot_verification.md"
...
if [[ -s "$MANIFEST_JSON" ]]; then
  while IFS= read -r line; do
    ...
    elif [[ "$name" != "backend_entitlement_status" ]]; then
...
{
  printf -- '# Payment Entitlement Staging Screenshot Verification\n\n'
  printf -- '- Upgrade: #057.0B\n'
  printf -- '- Status: %s\n' "$status"
...
printf -- 'Payment entitlement staging screenshot verification passed.\n'
