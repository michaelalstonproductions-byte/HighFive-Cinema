...
MANIFEST_JSON="$OUT_DIR/real_downloads_policy_staging_screenshot_manifest.json"
JSON_OUT="$OUT_DIR/real_downloads_policy_staging_screenshot_verification.json"
MD_OUT="$OUT_DIR/real_downloads_policy_staging_screenshot_verification.md"
...
if [[ -s "$MANIFEST_JSON" ]]; then
  while IFS= read -r line; do
    ...
    elif [[ "$name" != "backend_download_status" ]]; then
...
{
  printf -- '# Real Downloads Policy Staging Screenshot Verification\n\n'
  printf -- '- Upgrade: #059.0B\n'
  printf -- '- Status: %s\n' "$status"
...
printf -- 'Real downloads policy staging screenshot verification passed.\n'
