...
OUT_DIR="/private/tmp/highfive-phase-65-0a-staging-backend-scaffold"
mkdir -p "$OUT_DIR"
...
done < <(find "$SCAFFOLD_DIR/src" "$SCAFFOLD_DIR/test_contracts" -name '*.ts' -type f | sort)

for term in "${required_terms[@]}"; do
  if ! rg -q --fixed-strings "$term" "$SCAFFOLD_DIR" "$DOC_FILE"; then
...
if ! rg -q --fixed-strings "<SET_IN_STAGING_SECRET_STORE>" "$SCAFFOLD_DIR/env/highfive_staging_server.env.example"; then
  failures+=("env example missing secret-store placeholders")
fi
...
if git diff --name-only | rg -q 'HighFive/App/Depth|HighFive/App/Motion|HighFive/App/Playback|HighFive/App/Layer4|HighFive/App/Rendering|HighFive/App/Store|Assets.xcassets|Info.plist|PrivacyInfo|\.entitlements'; then
...
{
  printf '# HighFive Staging Backend Scaffold Verification\n\n'
  printf -- '- Upgrade: #065.0A\n'
  printf -- '- Status: %s\n' "$status"
