...
MANIFEST_JSON="$SCREENSHOT_DIR/watch_first_product_ux_screenshot_manifest.json"
JSON_OUT="$OUT_DIR/watch_first_product_ux_screenshot_verification.json"
MD_OUT="$OUT_DIR/watch_first_product_ux_screenshot_verification.md"
...
while IFS= read -r path; do
  paths+=("$path")
done < <(rg -o '"/private/tmp/highfive-phase-49-0b-watch-first-ux-evidence/screenshots/[^"]+\.png"' "$MANIFEST_JSON" | tr -d '"')
...
{
  printf -- '# Watch-First Product UX Screenshot Verification\n\n'
  printf -- '- Upgrade: #049.0B\n'
  printf -- '- Status: `%s`\n' "$status"
  printf -- '- Manifest: `%s`\n\n' "$MANIFEST_JSON"
